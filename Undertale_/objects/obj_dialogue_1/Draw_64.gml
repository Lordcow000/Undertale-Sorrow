...
if (!portrait)
{
	var _len = string_length(clean_text);
	
	for(l=1; l<= _len; l++)
	{
		randomise();
		var letter = string_char_at(clean_text,l);
		draw_text_transformed(text_x+l*15+(random_range(-1,1)), text_y+(irandom_range(-1,1)), letter, 2, 2, 0);
	}
